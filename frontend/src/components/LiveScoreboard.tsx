  ...
  return (
    <div className={`shrink-0 rounded-lg border px-3 py-1.5 ${live ? 'border-signal-green/50 bg-signal-green/5' : 'border-terminal-border bg-terminal-surface'}`}>
      <div className="flex items-center gap-2 font-mono text-xs">
        ...
        <span className={done || live ? 'text-signal-amber' : 'text-zinc-500'}>
          {g.away_score ?? '–'}
        </span>
      ...
      <div className="flex items-center gap-2 mt-0.5">
        ...
        <span className={`text-xs font-body ${live ? 'text-signal-green' : 'text-zinc-500'}`}>{g.detail}</span>
        {g.market_details && (
          <span className="text-xs font-mono text-zinc-500">· {g.market_details}{g.market_over_under ? ` O/U ${g.market_over_under}` : ''}</span>
        )}
      </div>
    </div>
  )
