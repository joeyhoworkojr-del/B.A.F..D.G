  ...
  return (
    <div className="flex items-center justify-between gap-2 py-1 border-b border-terminal-muted/30">
      <div className="min-w-0">
        ...
        <span className="ml-1.5 font-mono text-xs text-zinc-500">{player.position}</span>
      </div>
      <div className="flex gap-1">
        {STATUS_ORDER.map(s => (
          <button
            ...
            onClick={() => onSet(s)}
            className={`
              rounded border px-1.5 py-0.5 text-xs font-display uppercase tracking-wide transition-colors
              ${player.status === s ? STATUS_STYLE[s] : 'border-terminal-muted text-zinc-500 hover:text-zinc-400'}
            `}
          >
            {s}
          </button>
...
function TeamColumn({
  ...
  return (
    <div>
      <p className="text-xs font-display uppercase tracking-widest text-zinc-500 mb-1">{label}</p>
      {players.length === 0 ? (
        <p className="text-xs text-zinc-500 font-body">No tracked key players.</p>
      ) : (
        players.map(p => (
